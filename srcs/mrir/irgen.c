...
iblk_t visit_bop(igres_tp res, node_t node, mem_t igmem, ictx_t ictx);
...
iblk_t visit_nod(igres_tp res, node_t node, mem_t igmem, ictx_t ictx)
{
    ...
    case BOP_N:
        return visit_bop(res, node, igmem, ictx);
    }
}
...
iblk_t visit_bol(node_t node)
{
    ...
    if (nod->_stat)
        return iblk_set2(BOL_I, BOL_V, 0, SET_PROP(0, 0, 1, 1));
    return iblk_set2(BOL_I, BOL_V, 0, SET_PROP(0, 0, 1, 1));
}
...
{
    if (!node._nod)
        return iblk_set2(LST_I, LST_V, 0, SET_PROP(0, 0, 1, 0));
    ...
    lst_i blk = ilst_set(igmem, elms, nod->_siz);

    if (is_useful)
    {
        if (is_constexpr)
            return iblk_set1(LST_I, blk, LST_V, ++res->_data._lst_cnt, SET_PROP(1, 1, 1, 0));
        return iblk_set1(LST_I, blk, LST_V, ++res->_data._lst_cnt, SET_PROP(1, 1, 0, 0));
    }

    if (is_constexpr)
        return iblk_set1(LST_I, blk, LST_V, ++res->_data._lst_cnt, SET_PROP(0, 1, 1, 0));
    return iblk_set1(LST_I, blk, LST_V, ++res->_data._lst_cnt, SET_PROP(0, 1, 0, 0));
}

iblk_t visit_tpl(igres_tp res, node_t node, mem_t igmem, ictx_t ictx)
{
    if (!node._nod)
        return iblk_set2(TPL_I, TPL_V, 0, SET_PROP(0, 0, 1, 0));
    ...
    tpl_i blk = itpl_set(igmem, elms, nod->_siz);

    if (is_useful)
    {
        if (is_constexpr)
            return iblk_set1(TPL_I, blk, TPL_V, ++res->_data._tpl_cnt, SET_PROP(1, 1, 1, 0));
        return iblk_set1(TPL_I, blk, TPL_V, ++res->_data._tpl_cnt, SET_PROP(1, 1, 0, 0));
    }

    if (is_constexpr)
        return iblk_set1(TPL_I, blk, TPL_V, ++res->_data._tpl_cnt, SET_PROP(0, 1, 1, 0));
    return iblk_set1(TPL_I, blk, TPL_V, ++res->_data._tpl_cnt, SET_PROP(0, 1, 0, 0));
}

iblk_t visit_dct(igres_tp res, node_t node, mem_t igmem, ictx_t ictx)
{
    if (!node._nod)
        return iblk_set2(DCT_I, DCT_V, 0, SET_PROP(0, 0, 1, 0));
    ...
    dct_i blk = idct_set(igmem, keys, vals, nod->_siz);

    if (is_useful)
    {
        if (is_constexpr)
            return iblk_set1(DCT_I, blk, DCT_V, ++res->_data._dct_cnt, SET_PROP(1, 1, 1, 0));
        return iblk_set1(DCT_I, blk, DCT_V, ++res->_data._dct_cnt, SET_PROP(1, 1, 0, 0));
    }

    if (is_constexpr)
        return iblk_set1(DCT_I, blk, DCT_V, ++res->_data._dct_cnt, SET_PROP(0, 1, 1, 0));
    return iblk_set1(DCT_I, blk, DCT_V, ++res->_data._dct_cnt, SET_PROP(0, 1, 0, 0));
}
...
iblk_t visit_bop(igres_tp res, node_t node, mem_t igmem, ictx_t ictx)
{
    ...
    iblk_t op1 = visit_nod(res, nod->_op1, igmem, ictx);

    if (res->_herr)
        return op1;

    iblk_t op2 = visit_nod(res, nod->_op2, igmem, ictx);
    ...
    {
    case ADD_T:
        return ival_add(res, op1, op2, igmem, ictx, node._pss, node._pse);
    }
}
