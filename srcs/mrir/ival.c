...
iblk_t list_append(mem_t mem, iblk_t op1, iblk_t op2);
...
iblk_t ival_add(igres_tp res, iblk_t op1, iblk_t op2, mem_t mem, ictx_t ictx, pos_t pss, pos_t pse)
{
    ...
    if (op1._dtyp == LST_V)
    {
        ...
        {
            if (OPT_LVL && !IS_USEFUL(op1._prop) && !IS_USEFUL(op2._prop))
                return list_append(mem, op1, op2);

            bop_i blk = ibop_set(mem, op1, op2, "addLI");

            return iblk_set1(BOP_I, blk, LST_V, SET_PROP(1, 0, 0));
        }
        ...
        if (op2._dtyp == FLT_V)
        {
            ...
            bop_i blk = ibop_set(mem, op1, op2, "addLF");
        ...
        if (op2._dtyp == BOL_V)
        {
            ...
            bop_i blk = ibop_set(mem, op1, op2, "addLB");
        ...
        if (op2._dtyp == STR_V)
        {
            ...
            bop_i blk = ibop_set(mem, op1, op2, "addLS");
        ...
        if (op2._dtyp == LST_V)
        {
            if (OPT_LVL && !IS_USEFUL(op1._prop) && !IS_USEFUL(op2._prop))
            {
                ...
                uint64 siz = op1b->_siz + op2b->_siz;
                iblk_tp elms = blk_alloc(mem, siz * sizeof(iblk_t), MEM_SIZ);

                uint64 i, j;
                for (i = 0; i < op1b->_siz; i++)
                    elms[i] = op1b->_elms[i];
                for (j = 0; j < op2b->_siz; j++)
                    elms[i++] = op2b->_elms[j];

                lst_i blk = ilst_set(mem, elms, siz);
    ...
    if (op1._dtyp == DCT_V)
    {
        if (op2._dtyp == DCT_V)
        {
            if (OPT_LVL && !IS_USEFUL(op1._prop) && !IS_USEFUL(op2._prop))
            {
                ...
                dct_i blk = idct_set(mem, keys, vals, siz);
            ...
            return iblk_set1(BOP_I, blk, DCT_V, SET_PROP(1, 0, 0));
        }
